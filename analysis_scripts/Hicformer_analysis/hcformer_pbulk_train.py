    ...
    with torch.no_grad():
        with tqdm(total=len(data_loader), dynamic_ncols=True) as t:
            ...
            for i, item in enumerate(data_loader):
                if i == len(data_loader) - 1 and item[0].shape[0] < args.batch_size:
                    print("Skipping last batch as it's smaller than batch size")
                    continue
    ...
    for epoch in range(args.epochs):
        ...
        with tqdm(total=len(train_loader), dynamic_ncols=True) as t:
            ...
            for i, item in enumerate(train_loader):
                if i == len(train_loader) - 1 and item[0].shape[0] < args.batch_size:
                    print("Skipping last batch as it's smaller than batch size")
                    continue
...
if __name__=='__main__':
    ...
    # project name
    parser.add_argument('--proj', default='hcformer_pbulk_AD')
    ...
    if args.use_wandb:
        if args.use_sweep:
            sweep_name = str(args.proj)+str(args.num)
            sweep_configuration = {
                'project': str(args.proj),
                'method': 'random',
                'name': sweep_name,
                'parameters':{
                    'lr':{
            ...
            sweep_id = wandb.sweep(sweep=sweep_configuration, project=str(args.proj))
            wandb.agent(sweep_id, function=train)
        elif args.parallelize:
            args.model_save_path = os.path.join(args.model_save_path, str(args.proj)+str(args.num))
