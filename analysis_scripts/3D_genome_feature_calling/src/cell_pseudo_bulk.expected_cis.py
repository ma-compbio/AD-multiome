...
def main():
    ...
    for target_folder in target_folder_list:
        for cell_type in cell_type_list:
            ...
            p_list.append(p)
            # add 100kb resolution to the job list
            p = pool.submit(process_single_cool_file, cool_file, '100000', args.view_file)
            p_list.append(p)
    # do the job
    cmd_list = []
...
def process_single_cool_file(cool_file, res, view_file):
    output_file = cool_file.replace(".mcool", ".expected_cis.%dkb.tsv" % (int(res)/1000))
    cmd = "cooltools expected-cis %s::resolutions/%s --smooth --nproc 18 -o %s --view %s" % (cool_file, res, output_file, view_file)
    return cmd
