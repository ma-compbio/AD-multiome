...
# Last-modified: 16 Jan 2025 07:01:16 PM
...
def main():
    ...
    bam_list = []
    for bam_filename in config['bam_list'].split(","):
        bam_file = os.path.join(config['result_folder'], bam_filename, f"{config['library_id']}_human_hg38.bam")
        if not os.path.exists(bam_file):
            print(f"Error: {bam_file} not exists") 
            exit(1)
    ...
    #
    out_script_file = os.path.join(out_script_folder, f"{config['library_id']}_merge.sbatch.sh")
    out_log_file = os.path.join(out_script_folder, f"{config['library_id']}.log_merge")
    ...
    with open(out_script_file, 'w') as fout:
        ...
        print("#SBATCH --job-name merge_bam", file = fout)
        ...
        print("", file = fout)
        print("# run samtools merge", file = fout)
        merged_bam_file = os.path.join(result_folder, f"{config['library_id']}_human_hg38.bam")
        print("~/Software/samtools-1.9/bin/samtools merge -@ 14 -f %s %s" % (merged_bam_file, ' '.join(bam_list)), file = fout)
        print("", file = fout)
        #
