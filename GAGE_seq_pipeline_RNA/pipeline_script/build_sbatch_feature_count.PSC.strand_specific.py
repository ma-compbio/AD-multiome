...
def main():
    ...
    #
    out_script_file = os.path.join(out_script_folder, f"{config['library_id']}_featurecount_strand_spe.sbatch.sh")
    out_log_file = os.path.join(out_script_folder, f"{config['library_id']}.log_featurecount_strand_spe")
    result_folder = os.path.join(config['result_folder'], config['out_prefix'] + '.strand')
