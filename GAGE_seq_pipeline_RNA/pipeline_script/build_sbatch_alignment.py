...
def main():
    ...
    with open(out_script_file, 'w') as fout:
        ...
        print("", file=fout)
        print("#SBATCH -p ma1", file = fout)
        print("#SBATCH -w oven-0-25", file = fout)
        print("#SBATCH -t 2-00:00:00", file = fout)
        ...
        print("source ~/mambaforge/etc/profile.d/conda.sh", file = fout)
        print("conda activate ad_rna", file = fout)
        print("# go to working folder", file = fout)
        print("cd /work/magroup/shared/4DN_project/ad/pipeline_RNA/annotation", file = fout)
        print("", file = fout)
        print("# run STAR", file = fout)
        print(f"/work/magroup/shared/4DN_project/ad/pipeline_RNA/pipeline_script/RNA_alignment.sh -t {config['thread']} -l {config['library_id']} -f {config['R1']} -s {config['R2']} -i {config['STAR_genome']} -o {result_folder}", file = fout)
        print("", file = fout)
        #
